/* -------------------------------------------------------------------------- */
/* Node.js script which runs tests on each unit in ‘src’.                     */
/* -------------------------------------------------------------------------- */

//// Capture `fail()` calls.
global.alert = msg => global.msg = msg
...
const
 ...
 ,testFiles = [
   ...
   ,require('./unit-tests/life-cli.test.js')(config)
  ]
...
for (i=0; tests=testFiles[i]; i++) {
  passes = fails = 0
  results.push(`${BOLD}${tests.FILE}${END}  ${tests.UNIT}`)
